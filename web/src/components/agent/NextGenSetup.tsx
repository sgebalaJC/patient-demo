...
import {
  getNextGenStatus,
  saveNextGenCredentials,
  getNextGenAuthUrl,
  setNextGenEnabled,
  disconnectNextGen,
  type NextGenIntegration,
} from '../../lib/nextgen';
...
export const NextGenSetup: React.FC<Props> = ({ onStateChange }) => {
  const [searchParams, setSearchParams] = useSearchParams();
  const [integration, setIntegration] = useState<NextGenIntegration | null>(null);
  ...
  useEffect(() => {
    const errParam = searchParams.get('nextgen_error');
    const statusParam = searchParams.get('nextgen_status');
    if (errParam) setError(decodeURIComponent(errParam));
    if (statusParam === 'connected') setSuccess('NextGen connected successfully');
    if (errParam || statusParam) {
      const next = new URLSearchParams(searchParams);
      next.delete('nextgen_error');
      next.delete('nextgen_status');
  ...
  async function loadStatus() {
    setLoading(true);
    try {
      const data = await getNextGenStatus();
      setIntegration(data);
  ...
  async function handleSaveCreds() {
    ...
    setSaving(true);
    try {
      await saveNextGenCredentials(clientId.trim(), clientSecret.trim(), sandbox);
      setClientSecret('');
      setShowEdit(false);
      await loadStatus();
    } catch (err) {
  ...
  async function handleAuthorize() {
    ...
    setAuthorizing(true);
    try {
      const { url } = await getNextGenAuthUrl();
    ...
    return (
      <div className="card p-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-sm font-semibold text-secondary-900">NextGen Healthcare</h3>
  ...
  return (
    <div className="card overflow-hidden">
      <div className="p-4 flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {integration?.sandbox && (
              <span className="text-xs px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 font-medium">Sandbox</span>
            )}
          </div>
          <p className="text-xs text-secondary-500 mt-0.5">
            {isAuthorized
              ? 'The admin agent can read NextGen patient and chart data when this integration is enabled'
              : 'Connect NextGen so the admin agent can query the practice EHR'}
          </p>
        </div>
      ...
      {error && <div className="border-t border-secondary-200 px-4 py-2 text-xs text-red-600 bg-red-50">{error}</div>}
      {success && <div className="border-t border-secondary-200 px-4 py-2 text-xs text-green-600 bg-green-50">{success}</div>}

      {showEdit && (
        <div className="border-t border-secondary-200 p-4 bg-secondary-50/50 space-y-3">
          <div>
            <p className="text-xs font-medium text-secondary-700 mb-1">NextGen OAuth Client</p>
          ...
          <label className="block">
            ...
            <input
              ...
              placeholder="NextGen-issued client ID"
          ...
          <label className="block">
            ...
            <input
              ...
              placeholder="NextGen-issued client secret"
              className="mt-1 block w-full text-xs px-3 py-2 rounded-md border border-secondary-200 focus:outline-none focus:ring-1 focus:ring-primary-500"
            />
          </label>
          <label className="inline-flex items-center gap-2">
            ...
            />
            <span className="text-xs text-secondary-700">Use sandbox (nativeapi.nextgen.com/nge/sandbox)</span>
          </label>
          <div className="flex items-center gap-2">
            <button
      ...
      {hasCreds && !showEdit && (
        <div className="border-t border-secondary-200 p-4 bg-secondary-50/50 space-y-3">
          {integration?.redirectUri && (
            <div>
              <p className="text-xs font-medium text-secondary-700 mb-1">Redirect URI (register in NextGen)</p>
              <div className="flex items-center gap-2">
      ...
      <ConfirmModal
        ...
        title="Disconnect NextGen"
        message="Disconnect NextGen? Stored credentials and tokens will be deleted."
