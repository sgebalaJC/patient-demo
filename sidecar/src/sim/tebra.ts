/**
 * Tebra (formerly Kareo) simulator. Reads from the shared
 * `simulation/drchrono/patients` pool, transformed into Tebra's
 * patient shape.
 */
...
function toTebra(p: any) {
  return {
    PatientID: p.id,
    FirstName: p.first_name || "",
    LastName: p.last_name || "",
    DateOfBirth: p.date_of_birth || null,
    Gender: (p.gender || "unknown").toLowerCase() === "female" ? "Female" : "Male",
    Email: p.email || null,
...
export async function simTebra(
  method: string,
  path: string,
  ...
  if (method === "GET" && path === "patients") {
    ...
    return json({
      Patients: matched.slice(0, limit).map(toTebra),
  ...
  if (method === "GET" && /^patients\/\d+$/.test(path)) {
    ...
    return json(toTebra(snap.data()));
  }

  return json(
    { error: `Simulated Tebra does not implement: ${method} ${path}` },
    501,
  );
}
