...
export const logAuditEvent = onCall({
  ...
  try {
    ...
    // Write structured audit log — Cloud Logging (long-term, ingest-friendly).
    logger.info('[AUDIT]', {
      audit: true,
      actorId: context.uid,
      actorRole,
    ...
    // Mirror to Firestore so the admin UI can query/filter without scraping
    // Cloud Logging. Doc id = autogenerated; fields kept flat for indexing.
    // `expiresAt` mirrors emitAudit's 90-day TTL — operator must enable
    // Firestore TTL policy on this field for it to take effect.
    // Fire-and-forget — Firestore failures must not break audit logging.
    ...
    try {
      ...
      // Already logged to Cloud Logging above; mirror failure is non-fatal.
      logger.warn('Audit Firestore mirror failed:', { message: (mirrorErr as Error).message });
    }
...
export const onBootstrapRequestCreated = onDocumentCreated({
  ...
  try {
    ...
    try {
      ...
      // ran. The post-commit emit is wrapped in its own try/catch that
      // only logs: once requestRef is `ready`, the client's onSnapshot
      // already fired and signInWithCustomToken is in flight — letting an
      // emitAudit failure trigger the outer catch would delete the auth
      // user the client is now signing in as.
      try {
        await emitAudit({
          actorId: 'bootstrap',
          actorRole: 'system',
        ...
        logger.warn('Bootstrap audit emit failed (non-fatal, bootstrap already committed):', {
...
// Installer wizard support — /admin/install in the deployed portal calls this.
export {
