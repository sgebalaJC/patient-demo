    ...
    <div className="p-6 overflow-y-auto h-full">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-secondary-500 mt-0.5">Channels and plugins configured on the agent</p>
        </div>
        <button
          onClick={loadConfig}
      ...
      <div className="space-y-6 max-w-2xl">
        ...
        <div>
          ...
          {channels.length === 0 ? (
            <p className="text-sm text-secondary-400">No channels configured</p>
          ) : (
            <div className="space-y-2">
              {channels.map((ch) => (
                <div key={ch.name} className="card p-4 flex items-center gap-3">
                  <span
                    className={`h-2 w-2 rounded-full ${ch.enabled ? 'bg-green-500' : 'bg-secondary-300'}`}
                  />
                  <div className="flex-1">
                    <span className="text-sm font-medium text-secondary-900 capitalize">{ch.name}</span>
                  </div>
                  <span
        ...
        <div>
          ...
          {plugins.length === 0 ? (
            <p className="text-sm text-secondary-400">No plugins configured</p>
          ) : (
            <div className="space-y-2">
              {plugins.map((pl) => (
                <div key={pl.name} className="card p-4 flex items-center gap-3">
                  <span
                    className={`h-2 w-2 rounded-full ${pl.enabled ? 'bg-green-500' : 'bg-secondary-300'}`}
                  />
