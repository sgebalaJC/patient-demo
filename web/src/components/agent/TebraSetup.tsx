...
import {
  getTebraStatus,
  saveTebraCredentials,
  getTebraAuthUrl,
  setTebraEnabled,
  disconnectTebra,
  type TebraIntegration,
} from '../../lib/tebra';
...
export const TebraSetup: React.FC<Props> = ({ onStateChange }) => {
  const [searchParams, setSearchParams] = useSearchParams();
  const [integration, setIntegration] = useState<TebraIntegration | null>(null);
  ...
  useEffect(() => {
    const errParam = searchParams.get('tebra_error');
    const statusParam = searchParams.get('tebra_status');
    if (errParam) setError(decodeURIComponent(errParam));
    if (statusParam === 'connected') setSuccess('Tebra connected successfully');
    if (errParam || statusParam) {
      const next = new URLSearchParams(searchParams);
      next.delete('tebra_error');
      next.delete('tebra_status');
  ...
  async function loadStatus() {
    setLoading(true);
    try {
      const data = await getTebraStatus();
      setIntegration(data);
  ...
  async function handleSaveCreds() {
    ...
    setSaving(true);
    try {
      await saveTebraCredentials(clientId.trim(), clientSecret.trim());
      setClientSecret('');
      setShowEdit(false);
      await loadStatus();
    } catch (err) {
  ...
  async function handleAuthorize() {
    ...
    setAuthorizing(true);
    try {
      const { url } = await getTebraAuthUrl();
    ...
    return (
      <div className="card p-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-sm font-semibold text-secondary-900">Tebra (Kareo)</h3>
    ...
    <div className="card overflow-hidden">
      ...
      {showEdit && (
        <div className="border-t border-secondary-200 p-4 bg-secondary-50/50 space-y-3">
          <div>
            <p className="text-xs font-medium text-secondary-700 mb-1">Tebra OAuth Client</p>
          ...
          <label className="block">
            ...
            <input
              ...
              placeholder="Tebra-issued client ID"
          ...
          <label className="block">
            ...
            <input
              ...
              placeholder="Tebra-issued client secret"
              className="mt-1 block w-full text-xs px-3 py-2 rounded-md border border-secondary-200 focus:outline-none focus:ring-1 focus:ring-primary-500"
            />
          </label>
          <div className="flex items-center gap-2">
      ...
      {hasCreds && !showEdit && (
        <div className="border-t border-secondary-200 p-4 bg-secondary-50/50 space-y-3">
          {integration?.redirectUri && (
            <div>
              <p className="text-xs font-medium text-secondary-700 mb-1">Redirect URI (register in Tebra)</p>
              <div className="flex items-center gap-2">
      ...
      <ConfirmModal
        ...
        title="Disconnect Tebra"
        message="Disconnect Tebra? Stored credentials and tokens will be deleted."
