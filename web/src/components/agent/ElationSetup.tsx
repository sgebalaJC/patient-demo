...
import {
  getElationStatus,
  saveElationCredentials,
  getElationAuthUrl,
  setElationEnabled,
  disconnectElation,
  type ElationIntegration,
} from '../../lib/elation';
...
export const ElationSetup: React.FC<Props> = ({ onStateChange }) => {
  const [searchParams, setSearchParams] = useSearchParams();
  const [integration, setIntegration] = useState<ElationIntegration | null>(null);
  ...
  useEffect(() => {
    const errParam = searchParams.get('elation_error');
    const statusParam = searchParams.get('elation_status');
    if (errParam) setError(decodeURIComponent(errParam));
    if (statusParam === 'connected') setSuccess('Elation connected successfully');
    if (errParam || statusParam) {
      const next = new URLSearchParams(searchParams);
      next.delete('elation_error');
      next.delete('elation_status');
  ...
  async function loadStatus() {
    setLoading(true);
    try {
      const data = await getElationStatus();
      setIntegration(data);
  ...
  async function handleSaveCreds() {
    ...
    setSaving(true);
    try {
      await saveElationCredentials(clientId.trim(), clientSecret.trim(), sandbox);
      setClientSecret('');
      setShowEdit(false);
      await loadStatus();
    } catch (err) {
  ...
  async function handleAuthorize() {
    ...
    setAuthorizing(true);
    try {
      const { url } = await getElationAuthUrl();
    ...
    return (
      <div className="card p-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-sm font-semibold text-secondary-900">Elation Health</h3>
    ...
    <div className="card overflow-hidden">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-secondary-500 mt-0.5">
            {isAuthorized
              ...
              : 'Connect Elation so the admin agent can query the practice EHR'}
          </p>
        </div>
      ...
      {showEdit && (
        <div className="border-t border-secondary-200 p-4 bg-secondary-50/50 space-y-3">
          <div>
            <p className="text-xs font-medium text-secondary-700 mb-1">Elation OAuth Client</p>
            <p className="text-xs text-secondary-500 mb-3">
              Create an OAuth application in Elation's developer portal. Use the sandbox toggle to connect to the test
              environment.
            </p>
          </div>
          <label className="block">
            ...
            <input
              ...
              placeholder="Elation-issued client ID"
          ...
          <label className="block">
            ...
            <input
              ...
              placeholder="Elation-issued client secret"
              className="mt-1 block w-full text-xs px-3 py-2 rounded-md border border-secondary-200 focus:outline-none focus:ring-1 focus:ring-primary-500"
            />
          </label>
          <label className="inline-flex items-center gap-2">
            ...
            />
            <span className="text-xs text-secondary-700">Use sandbox (sandbox.elationemr.com)</span>
          </label>
          <div className="flex items-center gap-2">
            <button
      ...
      {hasCreds && !showEdit && (
        <div className="border-t border-secondary-200 p-4 bg-secondary-50/50 space-y-3">
          {integration?.redirectUri && (
            <div>
              <p className="text-xs font-medium text-secondary-700 mb-1">Redirect URI (register in Elation)</p>
              <div className="flex items-center gap-2">
      ...
      <ConfirmModal
        ...
        title="Disconnect Elation"
        message="Disconnect Elation? Stored credentials and tokens will be deleted."
