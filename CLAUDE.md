...
- **Admin SMS (Admin → SMS page):** compose/send + outbound/inbound history. Send routes through `sidecarProxy → sidecar /admin-api/messaging/send` — sim writes to `simulation/sms/outbound`, real hits Twilio REST and persists to `sms-outbound/{sid}`. Inbound replies land on the public sidecar route `/webhooks/twilio/inbound-sms` (HMAC-SHA1 verified against `TWILIO_AUTH_TOKEN`) and write to `sms-inbound/{sid}`. **Real mode needs `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_PHONE_NUMBER` in `/root/sidecar.env`** on the sidecar host, plus the webhook URL `http://<sidecar-host>:8081/webhooks/twilio/inbound-sms` registered on the Twilio phone number. Sim mode works without any of that. Implementation: `sidecar/src/real/messaging.ts` (real) + `sidecar/src/sim/messaging.ts` (sim).
